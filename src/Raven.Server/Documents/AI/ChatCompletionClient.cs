...
public class ChatCompletionClient : IDisposable
{
    ...
    public async Task<AiResponse> StreamingCompleteAsync(JsonOperationContext streamingContext, IMemoryContextPool contextPool,
        ...
        AiUsage usage, AiSchema schema, AiDebugTrace trace, CancellationToken token)
    {
        AddDefaultHeaders(request);
        using var streamedPropertyBuffer = new JsonOperationContextBuffer<byte>(streamingContext);

        bool structuredOutput = schema?.IsStructuredOutput != false;
        var alreadySeen = 0;
    ...
    public async Task<(string Result, string Message)> TestCompleteAsync(string systemPrompt, string userPrompt, string schema, CancellationToken token)
    {
        ...
        var aiSchema = AiSchema.Create(agentConfig: null, string.IsNullOrWhiteSpace(schema) ? null : new AiOutputOptions { OutputSchema = schema });
        var request = CreateCompletionRequest(context, [prompt, user], attachments: null, tools: null, useTools: false, streaming: false, aiSchema);
        var r = await CompleteAsync(context, request, new AiUsage(), aiSchema, trace: null, token);
        return (r.Result.ToString(), r.Message.ToString());
    }

    public async Task<AiResponse> CompleteAsync(JsonOperationContext context, HttpRequestMessage request, AiUsage usage, AiSchema schema, AiDebugTrace trace, CancellationToken token)
    {
        AddDefaultHeaders(request);
        ...
        var result = responseParser.GetContent(context, schema?.IsStructuredOutput != false);
    ...
    public void WriteCompletionRequestPayload(AsyncBlittableJsonTextWriter writer, JsonOperationContext ctx, IEnumerable<BlittableJsonReaderObject> messages, List<AiAttachment> attachments, List<BlittableJsonReaderObject> tools, bool useTools, bool streaming,
        AiSchema schema, string promptCacheKey = null)
    {
        ...
        {
            using (var stream = RecyclableMemoryStreamFactory.GetRecyclableStream(Encoding.UTF8.GetBytes(schema.Schema)))
            {
